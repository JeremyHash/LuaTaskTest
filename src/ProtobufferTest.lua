...
function lasterror() return c._last_error(P) end
...
function _reader:int(key) return c._rmessage_integer(self._CObj, key, 0) end

function _reader:real(key) return c._rmessage_real(self._CObj, key, 0) end

function _reader:string(key) return c._rmessage_string(self._CObj, key, 0) end

function _reader:bool(key) return c._rmessage_integer(self._CObj, key, 0) ~= 0 end

function _reader:message(key, message_type)
    local rmessage = c._rmessage_message(self._CObj, key, 0)
    if rmessage then
        local v = {_CObj = rmessage, _CType = message_type, _Parent = self}
        return setmetatable(v, _R_meta)
    end
end

function _reader:int32(key) return c._rmessage_int32(self._CObj, key, 0) end

function _reader:int64(key) return c._rmessage_int64(self._CObj, key, 0) end

function _reader:int52(key) return c._rmessage_int52(self._CObj, key, 0) end

function _reader:uint52(key) return c._rmessage_uint52(self._CObj, key, 0) end

function _reader:int_repeated(key)
    local cobj = self._CObj
    local n = c._rmessage_size(cobj, key)
    local ret = {}
    for i = 0, n - 1 do tinsert(ret, c._rmessage_integer(cobj, key, i)) end
    return ret
end
...
function _reader:real_repeated(key)
    ...
    local n = c._rmessage_size(cobj, key)
    local ret = {}
    for i = 0, n - 1 do tinsert(ret, c._rmessage_real(cobj, key, i)) end
    return ret
end
...
function _reader:string_repeated(key)
    ...
    local n = c._rmessage_size(cobj, key)
    local ret = {}
    for i = 0, n - 1 do tinsert(ret, c._rmessage_string(cobj, key, i)) end
    return ret
end
...
function _reader:bool_repeated(key)
    ...
    local n = c._rmessage_size(cobj, key)
    local ret = {}
    for i = 0, n - 1 do tinsert(ret, c._rmessage_integer(cobj, key, i) ~= 0) end
    return ret
end
...
function _reader:int32_repeated(key)
    ...
    local n = c._rmessage_size(cobj, key)
    local ret = {}
    for i = 0, n - 1 do tinsert(ret, c._rmessage_int32(cobj, key, i)) end
    return ret
end
...
function _reader:int64_repeated(key)
    ...
    local n = c._rmessage_size(cobj, key)
    local ret = {}
    for i = 0, n - 1 do tinsert(ret, c._rmessage_int64(cobj, key, i)) end
    return ret
end
...
function _reader:int52_repeated(key)
    ...
    local n = c._rmessage_size(cobj, key)
    local ret = {}
    for i = 0, n - 1 do tinsert(ret, c._rmessage_int52(cobj, key, i)) end
    return ret
end
...
function _reader:uint52_repeated(key)
    ...
    local n = c._rmessage_size(cobj, key)
    local ret = {}
    for i = 0, n - 1 do tinsert(ret, c._rmessage_uint52(cobj, key, i)) end
    return ret
end
...
_reader[6] = function(msg)
    ...
    return function(self, key) return message(self, key, msg) end
end
...
function _writer:bool(k, v) c._wmessage_integer(self, k, v and 1 or 0) end
...
function _writer:int_repeated(k, v)
    for _, v in ipairs(v) do c._wmessage_integer(self, k, v) end
end

function _writer:real_repeated(k, v)
    for _, v in ipairs(v) do c._wmessage_real(self, k, v) end
end

function _writer:bool_repeated(k, v)
    for _, v in ipairs(v) do c._wmessage_integer(self, k, v and 1 or 0) end
end

function _writer:string_repeated(k, v)
    for _, v in ipairs(v) do c._wmessage_string(self, k, v) end
end
...
function _writer:int32_repeated(k, v)
    for _, v in ipairs(v) do c._wmessage_int32(self, k, v) end
end

function _writer:int64_repeated(k, v)
    for _, v in ipairs(v) do c._wmessage_int64(self, k, v) end
end

function _writer:int52_repeated(k, v)
    for _, v in ipairs(v) do c._wmessage_int52(self, k, v) end
end

function _writer:uint52_repeated(k, v)
    for _, v in ipairs(v) do c._wmessage_uint52(self, k, v) end
end
...
_writer[6] = function(msg)
    ...
    return function(self, key, v) return message(self, key, v, msg) end
end
...
local function _pattern_create(pattern)
    ...
    local pat = {CObj = cobj, format = table.concat(lua), size = 0}
...
local function default_table(typename)
    ...
    v = {
        __index = function(tb, key)
            ...
            if 'table' ~= type(ret) then return ret end
    ...
    for k, v in pairs(tbl) do
        if type(v) == "table" then
            ...
            if t == 6 then
                set_default(msg, v)
            elseif t == 128 + 6 then
                for _, v in ipairs(v) do set_default(msg, v) end
            end
        end
    end
...
function register(buffer) c._env_register(P, buffer) end
...
function extract(tbl)
    ...
    if type(typename) == "string" and type(buffer) == "string" then
        if check(typename) then expand(tbl) end
    end

    for k, v in pairs(tbl) do if type(v) == "table" then extract(v) end end
end

default = set_default

local pbFile = io.open("/lua/tracker.pb", "rb")
            ...
            cells = {
                {
                    lac_id = 6311,
                    cells = {
                        {mcc = 460, mnc = 0, cell_id = 83751, cell_rssi = 20},
                        {mcc = 460, mnc = 2, cell_id = 88751, cell_rssi = 18}
                    }
                }, {
                    lac_id = 6312,
                    cells = {
                        {mcc = 460, mnc = 0, cell_id = 83752, cell_rssi = 20},
                        {mcc = 460, mnc = 2, cell_id = 88752, cell_rssi = 18}
                    }
                }
            },
            ta = 1
...
local function decodeAndPrint(encodeStream)
    ...
    local decodeTable = decode("tracker.Client", encodeStream)
    log.info(tag..".decodeTable", decodeTable)
    if decodeTable then
        log.info(tag..".decode message_id", decodeTable.message_id,
                 type(decodeTable.message_id))
        if decodeTable.message_id == "DEV_LOGIN" then
            log.info(tag..".decode project_id", decodeTable.log_in.project_id)
            log.info(tag..".decode project_name",
                     decodeTable.log_in.project_name)
            log.info(tag..".decode script_version",
                     decodeTable.log_in.script_version)
            log.info(tag..".decode iccid", decodeTable.log_in.iccid)
            log.info(tag..".decode imsi", decodeTable.log_in.imsi)
            log.info(tag..".decode heart_interval",
                     decodeTable.log_in.heart_interval)
        elseif decodeTable.message_id == "DEV_LOCATION" then
            log.info(tag..".decode type", decodeTable.location.type)

            if decodeTable.location.gps_exist then
                log.info(tag..".decode gps_info longitude",
                         decodeTable.location.gps_info.longitude)
                log.info(tag..".decode gps_info latitude",
                         decodeTable.location.gps_info.latitude)
                log.info(tag..".decode gps_info degree",
                         decodeTable.location.gps_info.degree)
                log.info(tag..".decode gps_info speed",
                         decodeTable.location.gps_info.speed)
                log.info(tag..".decode gps_info viewed_sates",
                         decodeTable.location.gps_info.viewed_sates)
            end

            if decodeTable.location.cell_exist then
                for k, v in ipairs(decodeTable.location.cell_info.cells) do
                    if type(v) == "table" then
                        log.info(tag..".decode cell_info", k, "lac_id",
                                 v.lac_id)
                        if type(v.cells) == "table" then
                            for m, n in ipairs(v.cells) do
                                log.info(tag..".decode cell_info", k,
                                         "cells", m, n.mcc, n.mnc, n.cell_id,
                                         n.cell_rssi)
                            end
                        end
                    end
                end
                log.info(tag..".decode ta", decodeTable.location.cell_info.ta)
            end

            if type(decodeTable.location.status) == "table" then
                log.info(tag..".decode status shake",
                         decodeTable.location.status.shake)
                log.info(tag..".decode status charger",
                         decodeTable.location.status.charger)
                log.info(tag..".decode status acc",
                         decodeTable.location.status.acc)
                log.info(tag..".decode status gps",
                         decodeTable.location.status.gps)
                log.info(tag..".decode status rssi",
                         decodeTable.location.status.rssi)
                log.info(tag..".decode status vbat",
                         decodeTable.location.status.vbat)
                log.info(tag..".decode status charge_status",
                         decodeTable.location.status.charge_status)
            end
        end
    end
end
...
sys.taskInit(function()
    ...
    while true do
        ...
        -- 登录报文的封包和解包
        local encodeStr = encode("tracker.Client", logIn)
        log.info(tag..".encode logInStr", encodeStr:toHex())
        decodeAndPrint(encodeStr)

        -- GPS位置报文的封包和解包
        encodeStr = encode("tracker.Client", locationGPS)
        log.info(tag..".encode locationGPSStr", encodeStr:toHex())
        decodeAndPrint(encodeStr)

        -- LBS位置报文的封包和解包
        encodeStr = encode("tracker.Client", locationLBS)
        log.info(tag..".encode locationLBSStr", encodeStr:toHex())
        decodeAndPrint(encodeStr)
