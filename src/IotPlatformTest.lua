...
if LuaTaskTestConfig.iotplatformTest.onenetTest then

local tag3 = "OnenetTest"

local ProductId = "A7ukaalbY8"
local ProductSecret = "69H5w1erH9d1KaPTnyGRy+UZLY0dE/ORoaWhG20x+f8="
...
local function proc(onenet_mqttClient)
    ...
    while true do
        ...
        --接收到数据
        if result then
            log.info(tag3,"mqttInMsg.proc",data.topic,string.toHex(data.payload))
...
local function onenet_iot()
    while true do
        ...
        log.info(tag3,"mqtt连接成功")

        --订阅主题
        if onenet_subscribe() then
            log.info(tag3,"mqtt订阅成功")
            --循环处理接收和发送的数据
            while true do
                mqtt_ready = true
                if not proc(onenet_mqttClient) then log.error(tag3,"mqttTask.mqttInMsg.proc error") break end
            end
        else
            log.info(tag3,"mqtt订阅失败")
        end
        mqtt_ready = false
        --断开MQTT连接
