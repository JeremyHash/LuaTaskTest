...
local function proc(mqttClient)
    ...
    while true do
        ...
        --接收到数据
        if result then
            log.info(tag..".mqttInMsg.proc",data.topic,string.toHex(data.payload))
...
local function cbFnc(result,prompt,head,body)
    ...
    if result then
        if dat.code==0 then
            ...
            log.info(tag,"腾讯云注册设备成功:", body)
        else
            log.info(tag,"腾讯云设备注册失败:", body)
        end
        enrol_end = true
    end
end
    ...
    while true do
        ...
        elseif payload.encryptionType==1 then
            ...
            if tx_mqttClient:subscribe(txiot_subscribetopic, nil) then
                log.info(tag,"mqtt订阅SUCCESS")
                --循环处理接收和发送的数据
                while true do
                    mqtt_ready = true
                    if proc(tx_mqttClient) then 
                        log.info(tag,"接收信息SUCCESS")
                    else
                        log.error(tag,"接收信息FAIL") break
                    end
                end
            else
                log.info(tag,"mqtt订阅FAIL")
            end
            mqtt_ready = false
        --断开MQTT连接
